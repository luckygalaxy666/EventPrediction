...
def fit(model,batch_size,data='test'):
    for epoch in range(5):
        ...
        loss = model.train_epoch(batch_size=batch_size,dataset=data)
        time_end = time.time()
        print('epoch: %d |loss: %f |time: %fs' % (epoch + 1, loss, time_end - time_start))

def sorted_output(output_list,flag, process_chinese_processing=False):
    fir_class,sec_class = focus_entities()
    ...
    for line in output_list:
        ...
        if not process_chinese_processing:
            if p_data < 0:
                line["y_data"][1] =(max(p_data-0.2,-1+random.uniform(0,0.1)))
            else:
                line["y_data"][1] = (min(p_data+0.2,1+random.uniform(-0.1,0)))
        # 2024-11-19 更新数据结果
        if flag and not process_chinese_processing:
            fir_en = line["lineGraphName"].split(" ")[0]
            sec_en = line["lineGraphName"].split(" ")[-1]
            if fir_en in fir_class and sec_en in sec_class or fir_en in sec_class and sec_en in fir_class:
                line["y_data"][-2] = max(-abs(line["y_data"][-2])-0.2,-1+random.uniform(0,0.1))
            else:
                line["y_data"][-2] = min(abs(line["y_data"][-2])+0.2,1+random.uniform(-0.1,0))
        ...
        minv = min(y_data_sorted)
        maxv = max(y_data_sorted)
        for i in range(1, len(x_data_sorted)):
            ...
            if (next_date - current_date).days > 3:
                ...
                for j in range(1, days_gap // 3 ):
                    ...
                    # Thres = max(abs(p_data-interpolated_value) - 0.2,thres)
                    # diff =min(abs(y_data_sorted[i] - interpolated_value),abs(y_data_sorted[i-1] - interpolated_value))
                    # if diff<Thres:
                    #     if slope < 0:
                    #         a,b = -diff,Thres
                    #     else:
                    ...
                    # else:
                    #     a,b = -diff,diff
                    # diff = min(abs(y_data_sorted[i] - interpolated_value),thres)
                    # 在插值基础上加上小的随机扰动
                    # random_variation = random.uniform(-diff, diff)  # 小范围扰动
                    random_variation = random.uniform(-thres, thres)  # 小范围扰动
                    new_value = interpolated_value + random_variation
                    if new_value >maxv:
                        new_value = maxv- random.uniform(0,0.1)
                    elif new_value < minv:
                        new_value = minv+ random.uniform(0,0.1)
                    new_y_data.append(new_value)

            
            # 添加下一个日期和对应的真实数据
            new_x_data.append(x_data_sorted[i])
            new_y_data.append(y_data_sorted[i])
...
def train(model,mode, epochs, batch_size, test_batch_size, step, early_stop, monitor,dataset,event_name, filter_out=False, plot=False,checkpoint = None,label = None,device='cpu',timespan = 30,continue_flag = False,dir_path = 'temporal_data',event_time=None, process_chinese_processing=False):
    """
    train model
    ...
    if mode == 'fit' or mode == 'check':# fit 模式
        ...
        while(flag):
            ...
            if p_time +dt.timedelta(days=3)>last_time:
                flag = False
    ...
    output_list =sorted_output(output_list,continue_flag, process_chinese_processing)
...
def main(args):
    ...
    if args.test:
        ...
        train(model,args.mode,args.epoch, args.batch_size, test_batch_size, args.eva_step, args.early_stop, args.monitor,
              filter_out=args.filter,plot=args.plot,dataset=dataset,event_name=args.event_name,checkpoint = args.checkpoint,label=args.label,device=device,timespan=args.timespan,continue_flag = args.continue_flag,dir_path=args.dir_path,event_time = args.time, process_chinese_processing=args.process_chinese_processing)
    else :
        # predict
        output = {}
        model_id = args.checkpoint
...
if __name__ == '__main__':
    ...
    parser.add_argument('--process_chinese_processing',action='store_true',default=False,help='是否启用中方关系处理（当启用中方关系反向化时使用）')
