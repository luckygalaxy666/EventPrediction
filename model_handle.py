    ...
    def get_base_model_from_config(self, config=None, data= None):
        ...
        if self.model == 'cygnet':
            ...
            base_model = self.BaseModel(
                num_entity=data.num_entity if default else config['num_entity'],
                num_relation=data.num_relation if default else config['num_relation'],
                en_embedding=data.en_embedding,
                rel_embedding=data.rel_embedding,
                hidden_dim=100 if default else config['hidden_dim'],
...
def save_checkpoint(model: torch.nn.Module,
    ...
    torch.save({
            ...
            'model_data': model.data,
               ...
               full_path + 'model_params.pth')
    # if name == 'base_model':
    #     torch.save({
    #         'model_state_dict': model.state_dict(),
    #         'prev_predict_result': model.data.prev_result,
    #             },
    #            full_path + 'model_params.pth')
    # else:
    #     torch.save({
    #             'model_state_dict': model.state_dict(),
    #             'prev_predict_result': model.data.prev_result,
    #             'now_predict_result': model.data.now_result,
    #                 },
    #             full_path + 'model_params.pth')

def focus_entities():
    fir_class =["台积电","魏哲家","美国","英伟达","AI芯片","黄仁勋","亚利桑那州","特斯拉","辅助驾驶芯片","半导体"]
    sec_class =["中国","中方","中共","中央","主席","北京","习近平","华为","外交部"]
    return fir_class,sec_class
...
def load_checkpoint(checkpoint: str,
    ...
    data = DataLoader([],config['dataset'], type=args.dir_path,event_name = args.event_name)
    # TODO: load data
    # data.load()
    data.to(device)
    base_model = model_handle.get_base_model_from_config(config,data)
    opt = get_optimizer(args, base_model)
    model = model_handle.Model(base_model, data, opt)
    inform = torch.load(path + 'model_params.pth', map_location=device,weights_only=False)
    model.load_state_dict(inform['model_state_dict'])
    model.data = inform['model_data']
    model.data.predict_event = args.event_name
    model.data.to(device)
    # model.data.predict_time = args.time
    # if checkpoint == 'base_model':
    #     inform = torch.load(path + 'model_params.pth', map_location=device)
    #     model.load_state_dict(inform['model_state_dict'])
    #     model.data.prev_result = inform['prev_predict_result']
    # else:
    #     inform = torch.load(path + 'model_params.pth', map_location=device)
    #     model.load_state_dict(inform['model_state_dict'])
    #     model.data.prev_result = inform['prev_predict_result']
    #     model.data.now_result = inform['now_predict_result']
    model.to(device)
    return model
