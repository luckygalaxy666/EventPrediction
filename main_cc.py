...
def fit(model,batch_size,data='test',mode = None,label = None,output = None,s_time = None): 
    predict_flag = False
    for epoch in range(5):
        time_start = time.time()
        if epoch == 4:
            predict_flag = True
        output,loss = model.train_epoch(batch_size=batch_size,dataset=data,predict_flag = predict_flag,mode = mode,label = label,output = output,s_time = s_time)
        time_end = time.time()
...
def sorted_output(output_list):
    for line in output_list:
        ...
        # 拆分排序后的日期和数据
        line["x_data"], line["y_data"] = zip(*sorted_pairs)
...
def train(model,mode, epochs, batch_size, test_batch_size, step, early_stop, monitor,dataset,event_name, filter_out=False, plot=False,checkpoint = None,label = None,device='cpu',timespan = 30,continue_flag = False,dir_path = 'temporal_data',event_time=None):
    ...
    if mode == 'fit' or mode == 'check':# fit 模式
        ...
        if continue_flag or mode == 'check': # 如果是init后续的fit 或者check ,使用init模式下处理好的next_data
            ...
            output = fit(model,batch_size,data=data,mode = mode,label = label,output = output,s_time = dataset[0][-1])
        ...
        while(flag):
            ...
            output = fit(model,batch_size,data=data,mode = mode,label = label,output = output,s_time = s_time)
            s_time = pp_time
