...
class REGCNBase(nn.Module):
    def __init__(self,
        ...
        # self.static_entity_embed = torch.nn.Parameter(self.en_embedding, requires_grad=True)
        # self.static_relation_embed = torch.nn.Parameter(self.rel_embedding)
    ...
    def normalize(self, tensor):
        ...
        """
        return F.normalize(tensor).double() if self.layer_norm else tensor
    ...
    def forward(self,
        ...
        """
        '''
        edges代表一个时间窗口内的一系列子图，edge才是一个子图中的所有边，只有反向传播时才更新self.static_relation_embed，对evolved_relation_embed的修改不会影响self.static_relation_embed'''
        # initial evolved entity embedding with static entity embedding
        edges = [edge.to(self.device) for edge in edges]
        evolved_entity_embed = self.normalize(self.static_entity_embed).to(self.device)
        # evolved_entity_embed = evolved_entity_embed.double()
        # initial evolved relation embedding with static relation embedding
        evolved_relation_embed = self.static_relation_embed.to(self.device)
        self.to(self.device)
        ...
        for i, edge in enumerate(edges):
            ...
            avg_embed.to(self.device)
            current_relation_embed[index] += avg_embed
            # concat static and current relation embedding as the input of GRU
            relation_gru_input = torch.cat([self.static_relation_embed, current_relation_embed], dim=1).to(self.device)
            # relation_gru_input = relation_gru_input.to(self.device)
