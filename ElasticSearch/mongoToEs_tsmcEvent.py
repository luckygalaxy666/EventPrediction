...
MONGO_DB = MongoClient(host="121.48.163.69", port=27018, username="root", password="rootdb@")['2024-12-2-task']['TSMC-Event']
...
def mongo_to_es():
    ...
    for doc in cursor:
        # 提取所需字段
        content = converter.convert(doc.get("news_content"))  # 假设文档中有一个名为 "news_content" 的字段
        # content = doc.get("content")
        date =  doc.get("data")
        prime_relations = doc.get("relation_fixed", [])  # 默认值为空列表
        relations = []
        keywords = doc.get("keywords", "")  # 默认值为空字符串
        if content == " This page has no content to extract" or content == None or date == None or prime_relations == []:
            continue
        # 将 content 字段的繁体中文转换为简体中文
        content = converter.convert(content)
        # 将 relations 字段的繁体中文转换为简体中文
        for relation in prime_relations:
            relations.append({
                "firstEntity": converter.convert(relation["head"]),
                "secondEntity": converter.convert(relation["tail"]),
                "relation": converter.convert(relation["relation"])
            })
        data = {
            "content": content,
            "relations": relations,
