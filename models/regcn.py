    ...
    def train_epoch(self, batch_size=512,dataset='train',predict_flag = False):
        ...
        for i in tqdm(index):
            ...
            if i >= self.seq_len:
                ...
                edges = data[0:i]
            data_flag = False
            if data[i][:,:2].size(0) > MAXSIZE:
                data_flag = True
            for j in range(0,data[i][:,:2].size(0),MAXSIZE):
                evolved_entity_embed, evolved_relation_embed = self.model.forward(edges)
                
                end = min(j+MAXSIZE,data[i][:,:2].size(0))
                score,_ = self.decoder(evolved_entity_embed, evolved_relation_embed, data[i][j:end, :2])
                loss = self.loss(score, data[i][j:end, 2])
                loss.backward()
