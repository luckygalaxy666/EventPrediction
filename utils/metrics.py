...
def calculate_rank(scores: torch.Tensor,
    ...
    if get_top_10:
        #Get the ten indexes with the highest scores for each row
        top_10_score   = _[:,:10]
        top_10_indices = sindex[:, :10]
        return t_rank[torch.arange(scores.shape[0]), target_index],top_10_indices,-top_10_score
    if get_scores:
        # 获取 target_index 对应的分数
        target_scores = scores[torch.arange(scores.shape[0]), target_index]
        return t_rank[torch.arange(scores.shape[0]), target_index],target_scores
