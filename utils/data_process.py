...
def load_data(file: str,
    ...
    with open(file, encoding=encoding) as f:
        ...
        for line in content:
            ...
            if get_flag:
                entities_cnt[int(fact[0])] += 1
                entities_cnt[int(fact[2])] += 1

    data = torch.LongTensor(data)
    return data , entities_cnt
...
def load_embedding(file: str,encoding='utf-8'):
    data = []
    with open(file, encoding=encoding) as f:
        for line in f:
            numbers = [float(x) for x in line.strip().split()]
            # 将这些浮点数转换为一个张量，并添加到列表中
            tensor = torch.tensor(numbers)
            data.append(tensor)
    data_embedding = torch.stack(data,dim=0)
